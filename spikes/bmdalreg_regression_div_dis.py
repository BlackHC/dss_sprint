...
import torch
from bmdal_reg.data import DictDataset, ParallelDictDataLoader
...
def fit_model_pool_diversity(
    ...
    **config,
    ...
    train_dl = ParallelDictDataLoader(
        data,
        train_idxs.expand(n_models, -1),
        batch_size=batch_size,
        shuffle=True,
        adjust_bs=False,
        drop_last=True,
    )
    pool_dl = ParallelDictDataLoader(
        data,
        pool_idxs.expand(1, -1),
        batch_size=batch_size,
        shuffle=True,
        adjust_bs=False,
        drop_last=False,
    )
    valid_dl = ParallelDictDataLoader(
        data,
        valid_idxs.expand(n_models, -1),
    ...
    for i in range(n_epochs):
        ...
        for train_batch, pool_batch in zip(
            train_dl, chain.from_iterable(repeat(pool_dl))
        ):
            ...
            cov = torch.cov(
                dss_sprint.utils.persisted_buffer_list.T
            )  # shape: batch_size x batch_size
            # compute the entropy of the covariance matrix
            entropy = (
                torch.linalg.slogdet(cov + torch.eye(len(cov)) * 1e-30)[1]
                / 2
                / len(y_pool)
            )
            ...
            if False:
                entropy = (
                    torch.linalg.slogdet(
                        torch.diag(torch.diag(cov)) + torch.eye(len(cov)) * 1e-30
                    )[1]
                    / 2
                    / len(y_pool)
                    - entropy
            ...
            # compute the loss
            loss = (
                ((y_train - y_pred) ** 2).mean(dim=-1).mean(dim=-1).sum()
            )  # sum over n_models
            ...
            if lr_sched == "lin":
                ...
                peak_at = 0.1
                current_lr = lr * min(
                    (step / n_steps) / peak_at, (1 - step / n_steps) / (1 - peak_at)
                )
            else:
        ...
        with torch.no_grad():
            ...
            for batch in valid_dl:
                ...
                valid_sses = valid_sses + ((y_train - model(X_train)) ** 2).mean(
                    dim=-1
                ).sum(dim=-1)
            valid_rmses = (
                torch.sqrt(valid_sses / len(valid_idxs)).detach().cpu().numpy()
            )
            # for hook in hooks:
            #     hook.remove()
...
class NNDivDisRegressor:
    ...
    def predict(self, X, reduce: bool = True):
        ...
        dl = ParallelDictDataLoader(
            data,
            idxs.expand(self.n_models, -1),
            batch_size=8192,
            shuffle=False,
            adjust_bs=False,
            drop_last=False,
...
normal_model = NNDivDisRegressor(n_ensemble=3, entropy_weight=0.0, seed=50)
...
black_dot = mlines.Line2D(
    [], [], color="black", marker=".", linestyle="None", markersize=10, label="Train"
)
