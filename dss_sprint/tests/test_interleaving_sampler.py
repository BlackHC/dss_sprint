...
def test_interleaving_random_batch_sampler():
    ...
    for batch in sampler:
        ...
        assert all(0 <= index < len(dataset) for dataset, indices in zip(sampler.datasets, indices) for index in indices)

        data = sampler.demultiplex_data([sampler.multiplexed_dataset[i] for i in batch])
        # assert the length of each group is correct
        assert all(len(group) == batch_size for group, batch_size in zip(data, sampler.batch_sizes))
