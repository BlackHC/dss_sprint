...
from torch.utils.data import Sampler, BatchSampler, Dataset, ConcatDataset, default_collate
...
class InterleavedRandomBatchSampler(Sampler[list[int]]):
    ...
    def __init__(self, datasets: list[Dataset], batch_sizes: list[int]):
        ...
        self._cum_sum_dataset_sizes = [sum(dataset_sizes[:i]) for i in range(len(dataset_sizes) + 1)]
        self._cum_sum_batch_sizes = [sum(batch_sizes[:i]) for i in range(len(batch_sizes) + 1)]
    ...
    def __iter__(self) -> Iterator[list[int]]:
        ...
        batch_samplers = [
                             BatchSampler(RandomSampler(self.datasets[0]), batch_size=self.batch_sizes[0],
                                          drop_last=False)
                         ] + [
                             BatchSampler(
                                 RandomSampler(dataset, num_samples=num_batches * batch_size, replacement=True),
                                 batch_size=batch_size, drop_last=False)
                             for dataset, batch_size in zip(self.datasets[1:], self.batch_sizes[1:])
                         ]
        for batches in zip(*batch_samplers):
            yield [self._cum_sum_dataset_sizes[dataset_index] + index
    ...
    def demultiplex_indices(self, batch_indices: list[int]) -> list[list[int]]:
        ...
        """
        return [[index - self._cum_sum_dataset_sizes[dataset_index]
                 for index in batch_indices[self._cum_sum_batch_sizes[i]:self._cum_sum_batch_sizes[i + 1]]]
    ...
    def demultiplex_data(self, batch_data: list) -> list[list]:
        ...
        Demultiplexes the batched tensor into batches for each group.
        """
        return [batch_data[self._cum_sum_batch_sizes[i]:self._cum_sum_batch_sizes[i + 1]]
